...
import { Coins, Skull, Trophy, ArrowUp, ArrowDown, ArrowLeft, ArrowRight, Heart } from 'lucide-react';
...
export default function MazeGame() {
  ...
  useEffect(() => {
    const updateCanvasScale = () => {
      ...
      const maxWidth = Math.min(window.innerWidth - 40, 400); // Maximum width with padding
      const scale = maxWidth / (CELL_SIZE * MAZE_SIZE);
      setCanvasScale(scale);

      // Update canvas display size while maintaining internal resolution
      canvas.style.width = `${CELL_SIZE * MAZE_SIZE * scale}px`;
      canvas.style.height = `${CELL_SIZE * MAZE_SIZE * scale}px`;
    };
    ...
    const interval = setInterval(() => {
      setGameState(prev => {
        ...
        if (newWaterLevel >= 100) {
          return { ...prev, gameOver: true, score: prev.score - 3 }; // Deduct 3 points for losing
        }
  ...
  const resetGame = useCallback(() => {
    ...
    setGameState(prev => {
      const gamesWon = prev.gamesWon + (prev.won ? 1 : 0);
      ...
      return {
        ...
        maze: newMaze,
        score: prev.score, // Keep the running score
        gameOver: false,
        won: false,
        size: MAZE_SIZE,
        gamesWon,
  ...
  const movePlayer = useCallback((dx: number, dy: number) => {
    setGameState(prev => {
      ...
      // Deep clone the maze to avoid state mutations
      const newMaze = JSON.parse(JSON.stringify(prev.maze));
      const newCell = newMaze[newY][newX];
      let newScore = prev.score;
      let gameOver = false;
      let won = false;

      // Collect coin - immediately update score
      if (newCell.hasCoin) {
        newScore += 1; // Add 1 point for collecting a coin
        newCell.hasCoin = false; // Remove the coin
      }

      // Check trap
      if (newCell.hasTrap && showTraps) {
        gameOver = true;
        newScore -= 3; // Deduct 3 points for losing
      }

      // Check goal
      if (newCell.isGoal) {
        won = true;
        newScore += 5; // Add 5 points for winning
      }
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center p-4">
      <div className="bg-white rounded-lg p-6 shadow-xl max-w-full">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 max-w-[200px] mx-auto">
          ...
          <button
            ...
            className="p-3 bg-gray-200 rounded flex items-center justify-center hover:bg-gray-300 active:bg-gray-400"
          >
            <ArrowUp size={24} />
          </button>
          <div></div>
          ...
          <div className="mt-4 text-center">
            {gameState.gameOver ? (
              <div className="flex items-center justify-center gap-2 text-red-500">
                <Skull />
                <span className="text-xl font-bold">Game Over! (-3 points)</span>
              </div>
            ) : (
        ...
        <div className="mt-4 text-sm text-gray-600">
          <div>Use arrow keys or on-screen controls to move</div>
      ...
      <footer className="mt-8 text-white flex items-center gap-2">
        Made with <Heart className="text-red-500" size={16} /> by Sruthi, 2025
      </footer>
    </div>
  );
}